...
1. SQL Editor에서 `supabase/migrations/202609150001_initial.sql`을 **한 번** 실행합니다. 이미 실행했다면 반복 실행하지 않습니다.
...
2026-09-15 확인 시 사용자 프로젝트의 테이블 생성과 비로그인 접근 거부는 확인했습니다. **Anonymous Sign-Ins는 아직 `false`로 응답**하여, 실제 학생 참가·Storage 왕복 검증은 이 설정을 켠 후 가능합니다. 클라이언트 연결 실패를 체험 데이터 저장 성공으로 처리하지 않습니다.
...
4. 계획·준비물·사진·비용·일기를 작성하고 **저장**을 누릅니다. 입력 중 변경 표시와 페이지 이탈 경고가 있습니다.
5. 각 국가의 도시, 날짜, 선택 이유, 일정, 활동, 일기, 알게 된 점을 모두 작성하면 최종 제출할 수 있습니다.
...
`projects → students → trips → destinations → packing_items / travel_photos`
