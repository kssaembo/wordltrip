...
**학생 세션:** 같은 학급 코드와 닉네임으로 다른 기기에서도 기존 기록에 접근합니다. 닉네임을 아는 같은 학급 사용자는 해당 기록에 접근할 수 있는 간편 접속 방식입니다. 공용 기기에서는 사용 후 로그아웃하세요. 여러 기기에서 동시에 수정하면 마지막 저장이 반영되므로 한 기기에서 작성하세요.
...
기존 설치는 `supabase/migrations/202609160003_rejoin_reopen.sql`을 한 번 실행한 뒤 프런트엔드를 업데이트하세요. 새 설치는 001 → 002 → 003 순서입니다.

- 학급 코드 + 정확히 같은 닉네임(앞뒤 공백 제외)으로 기존 기록에 연결합니다. 다른 학급의 같은 닉네임은 별도 학생입니다.
- 재접속은 새 여행을 만들거나 기존 사진/기록을 지우지 않습니다. 기존 브라우저의 연결도 유지됩니다. 브라우저 데이터 삭제 후에도 같은 정보로 재접속할 수 있습니다.
- `student_sessions`가 인증 사용자와 학생 기록을 연결합니다. 학생은 세션 연결을 직접 변경할 수 없으며 학급 참가 RPC로만 연결합니다. 기존 `students.user_id`는 생성자 정보이고 접근 권한은 세션으로 판단합니다.
- 교사 대시보드의 제출 취소 → 확인을 누르면 작성 중으로 바뀝니다. 담당 교사만 가능하며 기록과 사진은 유지됩니다. 학생은 새로고침 후 수정·저장·재제출합니다.
- 이미 작성 중인 화면은 저장되지 않은 내용 보호를 위해 자동 새로고침하지 않습니다.
